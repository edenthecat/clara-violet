 ...
 */
function i18n() {
	load_theme_textdomain( 'cv', CV_PATH . '/languages' );
 }
...
function scripts( $debug = false ) {
	...
	wp_enqueue_script(
		'cv',
		CV_TEMPLATE_URL . "/assets/js/clara-violet{$min}.js",
		array(),
		CV_VERSION,
		true
	);
...
function styles( $debug = false ) {
	...
	wp_enqueue_style(
		'cv',
		CV_URL . "/assets/css/clara-violet{$min}.css",
		array(),
		CV_VERSION
	);
}
...
function header_meta() {
	$humans = '<link type="text/plain" rel="author" href="' . CV_TEMPLATE_URL . '/humans.txt" />';

	echo apply_filters( 'cv_humans', $humans );
}
